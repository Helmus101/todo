...
4. **Credentials → Create credentials → OAuth client ID → Web application**. Add **Authorized redirect URIs**:
   - Local: `http://localhost:5273/auth/google/callback`
...
```bash
npm run dev      # Vite on :5273 (proxying API to the Express server on :8788)
# open http://localhost:5273  → sign up / log in → Connect Google
# (5273/8788 are deliberately distinct from the Electron desktop app's 5173/8787 — fully independent.)
```

`PUBLIC_URL` should be `http://localhost:5273` for local dev (matches the redirect URI above).
...
## Cloud persistence (per account)

Your profile, tasks, **and Google connection** are saved to **Supabase** keyed by your account email, so
they survive restarts/redeploys and follow your account — log back in and you're still connected to Google
with your tasks intact (no reconnecting).

> **Security:** the stored Google connection includes a refresh token, and account password hashes live in
> `weave_web_users`. With the **anon key** + the permissive policy in `supabase.sql` these are readable by
> anyone with that (semi-public) key — OK for personal use. For a real/shared deploy, set
> `SUPABASE_SERVICE_KEY` (the server prefers it; it bypasses RLS) and remove the permissive policies. Reuses Weave's existing Supabase project (`VITE_SUPABASE_URL` +
`VITE_SUPABASE_ANON_KEY` in `.env`). One-time: run **`web/supabase.sql`** in the Supabase SQL editor to
create the `weave_web_state` table. (For tighter security, set `SUPABASE_SERVICE_KEY` instead of the anon
key — it bypasses RLS so you can skip the permissive policy.) If Supabase isn't configured, the app still
works but keeps state in memory only (lost on restart).

## Opening tabs (optional Chrome extension)

Steps that mean "open a page" open a tab. With the tiny extension in `web/extension/`, Weave opens them
**unattended** and **collects a task's pages into one named tab group** (reusing the group as more pages
open, so they stay together) — plus an "Open all N pages ↗" button on a task. Load it:
`chrome://extensions` → enable **Developer mode** → **Load unpacked** → select `web/extension/`. (For a
deployed URL, add your domain to `content_scripts.matches` in its `manifest.json`.) Without the extension,
"open" steps fall back to `window.open` on a click — everything else still works.

**Test it:** click the extension's toolbar icon → a small popup lets you type a URL and open it in the
“Weave” tab group, to confirm tab-opening + grouping work.
