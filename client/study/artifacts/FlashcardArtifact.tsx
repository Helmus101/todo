...
import { FlashcardDeck, useLang } from "../../ui.tsx";
...
interface FlashcardArtifactProps {
  ...
  deckId: string;
  userId: string | null;
  // Threaded from ArtifactCanvas — closes this widget once every card has been answered correctly (retries
  // included). Optional so a caller without a real widget to close (none today) just skips auto-close.
  onClose?: () => void;
}
...
export function FlashcardArtifact({ task, deckId, userId, onClose }: FlashcardArtifactProps) {
  const L = useLang();
  ...
  const deck = task.flashcards?.find((f) => f.id === deckId) || getLocalDeck(deckId, userId) || undefined;
  if (!deck) return <div className="sm-artifact-empty">{L("Ce paquet n'est plus disponible.", "This deck is no longer available.")}</div>;
  return (
