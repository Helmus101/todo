...
/** Send a system-initiated email (a "new task" alert, or any future server-triggered notification).
 *  Bypasses the agent-tool gate since this is server-side work, not agent-decided. */
export async function sendSystemEmail(userId: string, opts: { to: string; subject: string; body: string; primaryAccounts?: Record<string, string> }): Promise<{ ok: boolean; error?: string }> {
  if (!integrationsReady() || !userId) return { ok: false, error: "Integrations not configured." };
  try {
    ...
    // Prefer the user's designated primary (Settings), else whichever account was connected first — mirrors
    // the exact fallback used everywhere else.
    let connectedAccountId: string | undefined;
    try {
