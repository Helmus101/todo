...
async function pronoteHomework(email, daysAhead = HOMEWORK_DAYS_AHEAD) {
...
function carryOverWeakCards(dayTasks, cap = 8) {
  const seen = /* @__PURE__ */ new Set();
  const out = [];
  ...
  for (const dt of sorted) {
    for (const deck of dt.flashcards || []) {
      for (const c of deck.cards) {
        ...
        const key3 = c.front.trim().toLowerCase();
...
var runViaJob = async (req, res, type, input) => {
  ...
  try {
    ...
    if (skipNote) {
      const msg = skipNote.includes("budget") ? BUDGET_MSG : /AI paused/i.test(skipNote) ? "AI is paused \u2014 resume it in Settings to run this." : /already executed|already handled/i.test(skipNote) ? "Otto's already run this one \u2014 nothing left to run." : /waiting for the user's Retry/i.test(skipNote) ? "This task failed before \u2014 use Retry instead of Run now." : "Couldn't run this right now \u2014 try again in a moment.";
...
app.post("/api/tasks/:id/quiz/:quizId/attempt", requireAuth, rateLimit(200, 6e4), ah(async (req, res) => {
  ...
  task.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (req.session.profile) req.session.profile.lastTutorActivityAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.post("/api/studylog/day", requireAuth, rateLimit(20, 6e4), ah(async (req, res) => {
  ...
  try {
    ...
    if (result) {
      ...
      const carried = carryOverWeakCards(priorDays);
      if (carried.length) {
