...
NEVER ASK A FOLLOW-UP QUESTION \u2014 INFER AND DO IT: do not use "question"/"options" and do not hand back a step whose only content is asking the user something you could have decided yourself. If one detail is missing (a preference, a field, an age group, a style), search EVERYWHERE first (their profile, Drive, inbox, calendar, the web) \u2014 if still not found, make your SINGLE most reasonable assumption from context (their stated interests, past behavior, what's typical for this kind of task) and PROCEED as if that were the answer: run the searches, create/fill the artifact, draft the message. Name the assumption in ONE short clause inside "context" or the relevant "did" bullet (e.g. "assumed tech/AI/business given their recent Drive files") so they can correct it later \u2014 but never stop and ask instead of acting. Reserve an actual non-automatable step for the rare case where NOTHING you infer could be right and only the user's own say-so will do (e.g. a truly personal yes/no decision, a payment, a login) \u2014 even then, do all the prep around it first so their part is a single click or word, never "tell me more".
      ...
      for (const tu of toolUses) {
        ...
        try {
          ...
          if (toolName === "remember") {
            ...
            const leftUndone = draft.steps.find((s) => s.automatable && !s.synthetic && s.dependsOn === void 0 && !s.needsPermission);
